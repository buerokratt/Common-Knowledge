...
class EestiSpider(BaseSpider):
    ...
    def get_existing_articles_from_db(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(existing_articles)} existing articles in database"
            )
            return existing_articles

        except Exception as e:
            self.logger.info(
                f"Error fetching existing articles (normal for first run): {e}"
            )
            return {}
...
<body>
    ...
    def should_process_article(
        ...
        if new_hash != existing_hash:
            ...
            requests.post(
                ...
                json={
                    "base_id": existing_articles[external_id].get("id"),
                    "status": "finished",
                },
            )
            return False

    def add_or_update_article_in_db(
        self, article_entry: Dict, existing_articles: Dict[str, Dict]
    ) -> Optional[str]:
        ...
        try:
            ...
            article_url = (
                ...
                if href
                else f"{self.base_url}/article/{external_id}"
            )
            ...
            if response.status_code == 200:
                ...
                self.logger.info(
                    f"Added new article {external_id} to database with ID {source_file_id}"
                )
                return source_file_id
            else:
                self.logger.error(
                    f"Failed to add article {external_id}: HTTP {response.status_code}"
                )
                return None
    ...
    def create_scrapped_item(
        self, article_data: Dict, article_entry: Dict, source_file_id: str
    ) -> ScrappedItem:
...
<body>
    ...
    {content}
        ...
        # Create file item
        file_item = FileItem(body=body_bytes, source_url=article_url, extension=".html")
    ...
    async def start(self) -> AsyncIterator[ScrappedItem]:
        ...
        """
        self.logger.info(
            "Starting EestiSpider - fetching from API and comparing with database"
        )
        ...
        for i, article_entry in enumerate(article_entries, 1):
            ...
            self.logger.info(
                f"Processing article {i}/{len(article_entries)}: ID {external_id}"
            )
            ...
            should_process = self.should_process_article(
                article_entry, article_data, existing_articles
            )
            ...
            source_file_id = self.add_or_update_article_in_db(
                article_entry, existing_articles
            )
            if not source_file_id:
                self.logger.error(
                    f"Failed to get source_file_id for article {external_id}"
                )
                continue

            # Create and yield the scraped item
            scraped_item = self.create_scrapped_item(
                article_data, article_entry, source_file_id
            )
            yield scraped_item
    ...
    async def parse(self, response: Response, **kwargs: object) -> None:
