...
class BaseSpider(Spider):
    ...
    def check_source_is_stopping(self) -> None:
        ...
        try:
            ...
            raise CloseSpider("source not found") from err
        if is_stopping:
    ...
    async def parse(
        ...
        if playwright_page:
            ...
            async with self.close_page(response) as page:
                ...
                is_sitemap = (
                    "sitemap" in response.url.lower() or file_extension == ".xml"
                )
                ...
                if file_extension == ".html" and not is_sitemap:
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Timeout waiting for content, proceeding anyway: {e}"
                        )
        ...
        file_item = FileItem(
            body=body_to_save, source_url=response.url, extension=file_extension
        )
        ...
        scrapped_item = ScrappedItem(
            file=file_item, metadata=metadata_item, hash=hashed
        )
