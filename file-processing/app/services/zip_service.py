...
def process_single_folder_zip(folder_item: FolderZipItem) -> FolderZipResult:
    ...
    try:
        ...
        logger.info(
            f"Downloading folder {clean_s3_path} to temporary location {local_folder_path}"
        )

        # Create exclusion filter function
        def exclusion_filter(relative_path: str) -> bool:
            return should_exclude_subfolder(
                relative_path, folder_item.excluded_folders or []
            )

        # Download the folder with exclusion filter
        successful_count, failed_count, download_results, excluded_paths = (
        ...
        for excluded_path in excluded_paths:
            excluded_subfolder = (
                excluded_path.split("/")[0] if "/" in excluded_path else excluded_path
            )
        ...
        if successful_count == 0:
            logger.info(
                f"No files found in {clean_s3_path}, creating zip with empty folder"
            )
        ...
        blob_storage_path = storage_provider.upload_file(
            temp_zip_path, clean_s3_zip_path
        )
        ...
        return FolderZipResult(
            ...
            files_count=max(
                successful_count, 1
            ),  # At least 1 for the empty folder case
...
def process_zip_task(task_id: str) -> None:
    ...
    try:
        ...
        for folder_item in task_data["folders"]:
            try:
                ...
                if result.status == "success":
                    ...
                    failed_zips += 1
                    logger.error(
                        f"Failed to zip folder {folder_item.s3_path}: {result.error_message}"
                    )

            except Exception as e:
                error_msg = (
                    f"Unexpected error processing {folder_item.s3_path}: {str(e)}"
                )
                results.append(
                    FolderZipResult(
                        s3_path=folder_item.s3_path,
        ...
        logger.info(
            f"Zip task {task_id} completed: {successful_zips} successful, {failed_zips} failed"
        )
        ...
        update_zip_task(task_id, status=TaskStatus.FAILED, error_message=error_msg)
