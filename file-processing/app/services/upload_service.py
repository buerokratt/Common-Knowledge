...
def process_task(task_id: str) -> None:
    ...
    try:
        ...
        full_source_path = os.path.join(
            settings.source_path, task_data["source_file_path"]
        )
        ...
        if not os.path.exists(full_source_path):
            update_task(
                task_id,
                TaskStatus.FAILED,
                error_message=f"Source file not found: {full_source_path}",
            )
            return

        destination_path = (
            f"uploads/{task_id}/{os.path.basename(task_data['source_file_path'])}"
        )

        blob_storage_path = storage_provider.upload_file(
            full_source_path, destination_path
        )
    ...
    try:
        ...
        for file_info in request.files:
            ...
            upload_url_tuples = storage_provider.generate_upload_urls(
                paths=[path], content_type=content_type, expires_in=request.expires_in
            )
            ...
            if upload_url_tuples:
                ...
                upload_url_items.append(
                    UploadUrlItem(
                        path=file_path, upload_url=upload_url, expires_at=expires_at
                    )
                )
...
def cleanup_old_tasks(max_age_hours: int = 24) -> int:
    ...
    tasks_to_remove = [
        task_id for task_id, task in _tasks.items() if task["updated_at"] < cutoff_time
    ]
