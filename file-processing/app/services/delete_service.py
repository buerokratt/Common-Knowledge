...
def process_single_file_delete(file_item: BlobFileDeleteItem) -> BlobFileDeleteResult:
    ...
    try:
        ...
        if success:
            return BlobFileDeleteResult(s3_path=file_item.s3_path, status="success")
        else:
            return BlobFileDeleteResult(
                s3_path=file_item.s3_path,
                status="failed",
        ...
        return BlobFileDeleteResult(
            s3_path=file_item.s3_path, status="failed", error_message=error_msg
        )
...
def process_delete_task(task_id: str) -> None:
    ...
    try:
        ...
        for file_item in task_data["files"]:
            try:
                if file_item.is_folder:
                    ...
                    logger.info(
                        f"Processed folder delete {file_item.s3_path}: {result.status}"
                    )
                else:
                    result = process_single_file_delete(file_item)
                    logger.info(
                        f"Processed file delete {file_item.s3_path}: {result.status}"
                    )
        ...
        logger.info(
            f"Delete task {task_id} completed: {successful_deletions} successful, {failed_deletions} failed"
        )
        ...
        update_delete_task(task_id, status=TaskStatus.FAILED, error_message=error_msg)
