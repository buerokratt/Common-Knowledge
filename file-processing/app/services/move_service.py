...
def process_single_file_move(file_item: FileMoveItem) -> FileMoveResult:
    ...
    try:
        ...
        if storage_provider.file_exists(clean_to_path):
            logger.warning(
                f"Destination file {clean_to_path} already exists, will be overwritten"
            )
...
def process_move_task(task_id: str) -> None:
    ...
    try:
        ...
        for file_item in task_data["files"]:
            try:
                if file_item.is_folder:
                    ...
                    logger.info(
                        f"Processed folder move {file_item.s3_from_path}: {result.status}"
                    )
                else:
                    result = process_single_file_move(file_item)
                    logger.info(
                        f"Processed file move {file_item.s3_from_path}: {result.status}"
                    )
                ...
                error_msg = (
                    f"Unexpected error processing {file_item.s3_from_path}: {str(e)}"
                )
                results.append(
                    FileMoveResult(
                        s3_from_path=file_item.s3_from_path,
        ...
        logger.info(
            f"Move task {task_id} completed: {successful_moves} successful, {failed_moves} failed"
        )
        ...
        update_move_task(task_id, status=TaskStatus.FAILED, error_message=error_msg)
