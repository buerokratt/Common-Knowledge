...
def process_folder_download(file_item: FileDownloadItem) -> FileDownloadResult:
    ...
    try:
        ...
        successful_count, failed_count, download_results, _excluded_paths = (
            storage_provider.download_folder(
        ...
        if failed_count == 0:
            return FileDownloadResult(
                ...
                status="success",
                file_size=sum(
                    os.path.getsize(result[1])
                    for result in download_results
                    if result[2] and os.path.exists(result[1])
                ),
                is_folder=True,
            ...
            return FileDownloadResult(
                ...
                local_path=local_folder_path,
                status="failed",
                error_message=f"All {total_files} files failed to download"
                if total_files > 0
                else "No files found in folder",
                is_folder=True,
                files_count=0,
            )
...
def process_download_task(task_id: str) -> None:
    ...
    try:
        ...
        for file_item in task_data["files"]:
            try:
                if file_item.is_folder:
                    ...
                    logger.info(
                        f"Processed folder download {file_item.s3_path}: {result.status}"
                    )
                else:
                    result = process_single_file_download(file_item)
                    logger.info(
                        f"Processed file download {file_item.s3_path}: {result.status}"
                    )
        ...
        logger.info(
            f"Download task {task_id} completed: {successful_downloads} successful, {failed_downloads} failed"
        )
        ...
        update_download_task(task_id, status=TaskStatus.FAILED, error_message=error_msg)
    ...
    try:
        ...
        if method == "GET":
            response = requests.get(
                callback.url, headers=headers, params=body_data, timeout=30
            )
        else:  # POST, PUT, PATCH, etc.
            response = requests.request(
                method, callback.url, headers=headers, json=body_data, timeout=30
            )

        if response.status_code < 400:
            logger.info(
                f"Callback executed successfully for task {task_id}: {response.status_code}"
            )
        else:
            logger.warning(
                f"Callback returned error status for task {task_id}: {response.status_code} - {response.text}"
            )
        ...
        logger.error(
            f"Unexpected error executing callback for task {task_id}: {str(e)}"
        )


def download_files_to_volume_async(
    request: DownloadToVolumeRequest,
) -> DownloadTaskResponse:
...
def delete_files_from_volume(
    request: DeleteFromVolumeRequest,
) -> DeleteFromVolumeResponse:
        ...
        try:
            ...
            if local_path.exists():
                if local_path.is_file():
                    ...
                    results.append(
                        FileDeleteResult(local_path=str(local_path), status="success")
                    )
                    ...
                    failed_deletions += 1
                    logger.error(
                        f"Failed to delete {local_path}: neither file nor directory"
                    )
            else:
                results.append(
                    FileDeleteResult(
