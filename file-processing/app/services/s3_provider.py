...
class S3Provider(BlobStorageProvider):
    ...
    def upload_file_content(
        ...
        try:
            ...
            raise BlobStorageError("AWS credentials not found") from err
    ...
    def generate_download_url(self, path: str) -> tuple[str, datetime]:
        try:
            ...
            expires_at = datetime.now(timezone.utc).replace(microsecond=0) + timedelta(
                seconds=settings.s3_presigned_url_expiration
            )
    ...
    def download_folder(
        ...
        exclusion_filter: Optional[Callable[[str], bool]] = None,
    ) -> Tuple[int, int, List[Tuple[str, str, bool, Optional[str]]], List[str]]:
        ...
        try:
            ...
            page_iterator = paginator.paginate(
                Bucket=self.bucket_name, Prefix=clean_prefix
            )
            ...
            for s3_key, _, last_modified in all_objects:
                try:
                    # Calculate relative path by removing the prefix
                    relative_path = (
                        s3_key[len(clean_prefix) :] if clean_prefix else s3_key
                    )
                    ...
                    if s3_key.endswith("/"):
                        ...
                        local_folder_full = os.path.join(
                            local_folder_path, folder_rel_path
                        )
                        ...
                        if os.path.exists(local_folder_full):
                            ...
                            results.append(
                                (s3_key, local_folder_full, False, "Folder not created")
                            )
                            failed_downloads += 1
                            results.append(
                                (s3_key, local_folder_full, False, "Folder not created")
    ...
    def move_folder(self, source_prefix: str, destination_prefix: str) -> bool:
        ...
        try:
            ...
            if not all_objects:
                ...
                self.s3_client.put_object(
                    Bucket=self.bucket_name, Key=destination_prefix, Body=b""
                )
                return True
            ...
            try:
                ...
                for source_key, _ in moved_objects:
                    ...
                    if self.file_exists(source_key):
                        logger.warning(
                            f"Source object {source_key} still exists after deletion"
                        )
    ...
    def delete_folder(self, prefix: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully deleted {deleted_count} objects from folder {prefix}"
            )
            ...
            for i in range(len(path_parts), 0, -1):
                ...
                try:
                    self.s3_client.head_object(
                        Bucket=self.bucket_name, Key=folder_marker
                    )

                    # Check if the folder is now empty (no other objects with this prefix)
                    response = self.s3_client.list_objects_v2(
                        Bucket=self.bucket_name, Prefix=folder_marker, MaxKeys=1
                    )
                    ...
                    if e.response["Error"]["Code"] == "404":
                        ...
                        logger.warning(
                            f"Error checking folder marker {folder_marker}: {str(e)}"
                        )
                        continue
